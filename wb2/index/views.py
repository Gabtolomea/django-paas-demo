...
def porter_in(request):
    ...
    for t in range(len(tablenames)):
        ...
        for c in range(mycursor.fetchone()[0]):
            mycursor.execute("SELECT " + columnnames[col] + " FROM "+tablenames2[t]+";")
...
def rearrange(var):
    arr = []
    mycursor.execute("SELECT count(*) FROM information_schema.columns WHERE TABLE_NAME = 'barangay_record';")
    cols = mycursor.fetchone()[0]
    for r in range(98):
        inner = []
        for c in range(cols):
            inner.append(barangay_record[c][r])
        arr.append(inner)
    print(arr)
