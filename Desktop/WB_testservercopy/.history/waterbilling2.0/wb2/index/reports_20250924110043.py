...
#     # Add consumer data
#     for row in table_data:
#         # Clean consumer name and parse numeric values
#         consumer_name = clean_string(row[0])
#         values = [float(row[i][1:].replace(",", "")) for i in range(1, len(row))]
#         ws.append([consumer_name] + values)

#     # Add total row
#     total_name = clean_string(total_row[0])
#     total_values = [float(total_row[i][1:].replace(",", "")) for i in range(1, len(total_row))]
#     ws.append([total_name] + total_values)
...
def parse_currency_to_float(value): # added
    try:
